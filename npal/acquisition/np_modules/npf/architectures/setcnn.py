...
from ..utils.helpers import ProbabilityConverter, backward_pdb, mask_and_apply
...
class UnsharedExpRBF(nn.Module):
    ...
    def __init__(self, x_dim, max_dist=1 / 256, max_dist_weight=0.99, p=2, **kwargs):
        super().__init__()
    ...
    def reset_parameters(self):
        ...
        max_dist_sigma = self.max_dist / ((-math.log(self.max_dist_weight)) ** (1 / self.p))
...
class ExpRBF(nn.Module):
    ...
    def forward(self, diff):
        ...
        inp = -(dist / sigma).pow(self.p)
        out = torch.softmax(inp, dim=-2)  # numerically stable normalization of the weights by density
...
class SetConv(nn.Module):
    ...
    def __init__(self, x_dim, in_channels, out_channels, RadialBasisFunc=ExpRBF, **kwargs):
        super().__init__()
