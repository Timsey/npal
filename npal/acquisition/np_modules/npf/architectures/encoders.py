...
from ..utils.helpers import mask_and_apply
...
class MergeFlatInputs(nn.Module):
    ...
    def __init__(self, FlatModule, x1_dim, x2_dim, n_out, is_sum_merge=False, **kwargs):
        ...
        if self.is_sum_merge:
            dim = x1_dim
            # NOTE: If we don't specify hidden_size here, then this will trigger hidden_size < input_size & output_size
            #  warning whenever the latter are > 32, since MLP default is hidden_size=32.
            # This MLP has a single hidden layer.
            self.resizer = MLP(x2_dim, dim, hidden_size=min(x2_dim, dim))  # transform to be the correct size
        else:
            dim = x1_dim + x2_dim
...
def merge_flat_input(module, is_sum_merge=False, **kwargs):
    ...
    def merged_flat_input(x_shape, flat_dim, n_out, **kwargs2):
        ...
        return MergeFlatInputs(module, x_shape, flat_dim, n_out, is_sum_merge=is_sum_merge, **kwargs2, **kwargs)
