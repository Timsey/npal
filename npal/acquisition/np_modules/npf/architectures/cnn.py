...
from ..utils.initialization import init_param_, weights_init
...
class ResConvBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.n_conv_layers == 2:
            ...
            self.conv1 = make_depth_sep_conv(Conv)(in_chan, in_chan, kernel_size, padding=padding, bias=is_bias)
        self.norm2 = Normalization(in_chan)
        self.conv2_depthwise = Conv(in_chan, in_chan, kernel_size, padding=padding, groups=in_chan, bias=is_bias)
...
class ResNormalizedConvBlock(ResConvBlock):
    ...
    def __init__(self, in_chan, out_chan, Conv, kernel_size=5, activation=nn.ReLU(), is_bias=True, **kwargs):
        super().__init__(
            in_chan,
            out_chan,
            Conv,
...
class UnetCNN(CNN):
    ...
    def _get_in_out_channels(self, n_channels, n_blocks):
        ...
        channel_list = channel_list[:1] + [min(c, self.max_nchannels) for c in channel_list[1:-1]] + channel_list[-1:]
        ...
        in_out_channels[idcs] = [(in_chan * 2, out_chan) for in_chan, out_chan in in_out_channels[idcs]]
