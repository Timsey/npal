...
class GetRandomIndcs:
    ...
    def __call__(self, batch_size, n_possible_points):
        ...
        if self.is_batch_share:
            ...
            indcs = np.arange(n_possible_points).reshape(1, n_possible_points).repeat(batch_size, axis=0)
...
class GridCntxtTrgtGetter(CntxtTrgtGetter):
    ...
    def __init__(self, context_masker=RandomMasker(), target_masker=no_masker, upscale_factor=1, **kwargs):
        self.upscale_factor = upscale_factor
        super().__init__(contexts_getter=context_masker, targets_getter=target_masker, **kwargs)

    def __call__(self, X, y=None, context_mask=None, target_mask=None, is_return_masks=False, **kwargs):
        """
        Parameters
        ----------
...
class SuperresolutionCntxtTrgtGetter(GridCntxtTrgtGetter):
    ...
    **kwargs :
    ...
    def __init__(self, resolution_factor=1 / 4, downsample_mode="area", **kwargs):
        ...
        super().__init__(
            context_masker=ResolutionMasker(factor=int(1 / self.resolution_factor)), target_masker=no_masker, **kwargs
        )

    def preprocess_context(self, X):
        """Preprocess the data for the context set."""
        X = channels_to_2nd_dim(X)
        X_downscale = F.interpolate(X, scale_factor=self.resolution_factor, mode=self.downsample_mode)
        # upscale but with "nearest neighbor" interpolation => keep low resolution
        X_lowres = F.interpolate(X_downscale, scale_factor=int(1 / self.resolution_factor), mode="nearest")
