...
def clamp(
    ...
    """
    lower_bound = (minimum + negative_slope * (x - minimum)) if is_leaky else torch.zeros_like(x) + minimum
    upper_bound = (maximum + negative_slope * (x - maximum)) if is_leaky else torch.zeros_like(x) + maximum
...
class ProbabilityConverter(nn.Module):
    ...
    def reset_parameters(self):
        ...
        initial_bias = self._probability_to_bias(self.initial_probability, initial_x=self.initial_x)
...
def make_depth_sep_conv(Conv):
    ...
    class DepthSepConv(nn.Module):
        ...
        **kwargs :
        ...
        def __init__(self, in_channels, out_channels, kernel_size, confidence=False, bias=True, **kwargs):
            super().__init__()
            self.depthwise = Conv(in_channels, in_channels, kernel_size, groups=in_channels, bias=bias, **kwargs)
