...
from ..architectures import CNN, ResConvBlock, SetConv, discard_ith_arg
...
class ConvCNP(NeuralProcessFamily):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "Decoder" in kwargs and kwargs["Decoder"] != nn.Identity:  # identity means that not using
            logger.warning(
    ...
    def encode_globally(self, X_cntxt, Y_cntxt):
        ...
        if n_cntxt == 0:
            ...
            R_induced = torch.zeros(batch_size, self.n_induced, self.r_dim, device=R_induced.device)
...
class ConvLNP(LatentNeuralProcessFamily, ConvCNP):
    ...
    def trgt_dependent_representation(self, X_cntxt, z_samples, R_induced, X_trgt):
        ...
        X_induced = collapse_z_samples_batch(replicate_z_samples(X_induced, n_z_samples))
        ...
        if self.encoded_path == "latent":
            ...
            # size = [n_z_samples, batch_size, n_induced, z_dim]
            z_samples = z_samples.expand(n_z_samples, batch_size, self.n_induced, self.z_dim)
    ...
    def add_global_latent(self, z_samples):
        ...
        local_z_samples, global_z_samples = z_samples.split(z_samples.shape[-1] // 2, dim=-1)
