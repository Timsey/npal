...
class LossNet(nn.Module):
    def __init__(self, feature_sizes=[32, 16, 8, 4], num_channels=[64, 128, 256, 512], interm_dim=128):
        ...
        self.pooling_layers = nn.ModuleList([nn.AvgPool2d(fs) for fs in feature_sizes])
        self.fc_layers = nn.ModuleList([nn.Linear(nc, interm_dim) for nc in num_channels])

        # # Pool features into single feature per channel
        # self.GAP1 = nn.AvgPool2d(feature_sizes[0])
        # self.GAP2 = nn.AvgPool2d(feature_sizes[1])
        # self.GAP3 = nn.AvgPool2d(feature_sizes[2])
        # self.GAP4 = nn.AvgPool2d(feature_sizes[3])
        #
        # # Map flattened channels to linear
    ...
    def forward(self, features):
        outs = []
        for ind in range(len(self.pooling_layers)):
            out = self.pooling_layers[ind](features[ind])
            out = out.view(out.size(0), -1)
            out = F.relu(self.fc_layers[ind](out))
            outs.append(out)
