...
class CNNClassifier(BaseClassifier):
    def __init__(self, args, classes, remaining_classes, imbalance_factors, suppress_train_log=False):
        ...
        self.logger = logging.getLogger("CNNClassifier")
        self.report_interval = self.num_epochs // 10
        ...
        if args.data_type in ["mnist", "fashion_mnist"]:
            ...
            raise NotImplementedError("No CNN implemented for non-MNIST data.")
        if not suppress_train_log:
            self.logger.info("CNN with {} params.".format(count_trainable_parameters(self.model)))
        ...
        # NOTE: Doesn't do anything if MILESTONES[0] = self.num_epochs.
        #  Implemented here for consistency with Learning Loss.
...
class SimpleCNN(nn.Module):
    """Parts of architecture copied from:
    ...
    def __init__(self, input_shape, num_channels, num_classes, learning_loss=False):
        ...
        if input_shape == (28, 28):  # MNIST, FashionMNIST
            self.feature_sizes = [14, 7]
        elif input_shape == (32, 32):  # SVHN, CIFAR10
            self.feature_sizes = [16, 8]
        else:
        ...
        self.out = nn.Linear(32 * self.feature_sizes[-1] * self.feature_sizes[-1], num_classes)

        self.lloss = None
        if learning_loss:
            self.lloss = LossNet(feature_sizes=self.feature_sizes, num_channels=[16, 32], interm_dim=128)
    ...
    def forward(self, x):
        ...
        # flatten the output of conv2 to e.g. for MNIST: (batch_size, 32 * 7 * 7)
        flat_pre_logits = out2.view(out2.size(0), -1)
        logits = self.out(flat_pre_logits)  # b x classes
        return logits, flat_pre_logits, [out1, out2]
